...
import BuildCntrols from '../../components/Burger/BuildControls/BuildControls';
...
class BurgerBuilder extends Component{
    ...
    componentDidMount(){
        axios.get('https://react-my-burger-c302f.firebaseio.com/ingredients.json')
            .then(response => {
    ...
    purchasContinueHandler = () => {
        ...
        for(let i in this.state.ingredients){
            queryParameter.push(encodeURIComponent(i)+ '=' + encodeURIComponent(this.state.ingredients[i]));   
        }
    ...
    removeIngredientHandler = (type) => {
        ...
        updatedIngredients[type] = updatedCount;
        const priceDelition = INGREDIENT_PRICES[type];
        const oldPrice = this.state.totalPrice;
        const newPrice = oldPrice - priceDelition;
        this.setState({totalPrice:newPrice, ingredients:updatedIngredients})
        this.updatePurchaseState(updatedIngredients);
